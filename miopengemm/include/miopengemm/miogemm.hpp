 ...
 * 
 * @param allotted_time       an upper bound in seconds on the time spent searching. When this time has expired, the best Solution found thus far is returned.  
 ...
 * @param enforce_determinism If true, only kernels which are bitwise consistent are considered. For small GEMM problems enforce_determinism = False finds faster Solutions.
 * @param tgg                 GEMM problem Geometry as in MIOpenGEMM/include/geometry.hpp
 * @param verbose             Print the search summary to std::cout
 * @param with_warnings       Print warnings to std::cerr. Warnings are provided when the allotted_time is insufficient to find good Solutions.
...
/*! @brief Find a Solution, considering kernels with and without workspace requirements. This function is experimental.
 * 
 * @param command_queue       OpenCL command queue 
 ...
 * @param constraints_string  String describing the constraints on the kernels considered. It is a substring of a hyperstring, defining with hyper-parameters have fixed values
 * @param gg                  GEMM problem Geometry as in MIOpenGEMM/include/geometry.hpp
...
/*! @brief Return a default Solution from cache, without exploring. If no Solution exists, an error will thrown.
 * 
 * @param command_queue       OpenCL command queue, used to extract device name
 * @param constraints_string  String describing the constraints on the kernels considered. It is a substring of a hyperstring, defining with hyper-parameters have fixed values
 * @param gg                  GEMM problem Geometry
 * @param k_comment           If a Solution from a custom cache is to be returned, this can be a non-empty string
 * @param mowri               Controls writing to terminal and file
 * @return                    best Solution found during search 
 */
 
Solution get_default(
  cl_command_queue                 command_queue,
...
/*! @brief Return a default Solution, independent of device. This version of get_default does not require a Solution to exist in cache.
 *
 * @param gg                  GEMM problem Geometry
 ...
 * @return                    A tuple, first argument is true if a Solution exists is cache. The second argument is a string describing, if a Solution does not exist, possible reasons.  
 */

...
void fill_uni(std::vector<TFloat>& v, unsigned r_small, unsigned r_big)
{
  ...
  {
    std::stringstream ss;
    ss << "in fill_uni, with r_small > r_big. this seems like a strange "
          "(incorrect) request ";
             ...
             const Offsets&       toff)
{
  ...
  size_t n_a = gg.ldX[nsHP::matA] * (gg.tX[nsHP::matA] == gg.isColMajor ? gg.m : gg.k) + toff.oa +
               toff.tail_off_a;
  size_t n_b = gg.ldX[nsHP::matB] * (gg.tX[nsHP::matB] == gg.isColMajor ? gg.k : gg.n) + toff.ob +
               toff.tail_off_b;
  ...
  if (n_a > 20000 * 10000 || n_b > 20000 * 10000 || n_c > 20000 * 10000)
  {
    ...
    ss << "currently, this code only generates random matrices of size less "
       << "than 20000*10000. "
       << "the request with geometry " << gg.get_string() << " is therefore rejected."

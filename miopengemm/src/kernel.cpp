...
openclutil::OpenCLResult Kernel::update(const KernelString & ks, outputwriting::OutputWriter & mowri){
  ...
  auto oclr = openclutil::cl_set_program_and_kernel(command_queue, tgk_strings.kernstr, tgk_strings.fname, clprog, clkern, mowri, false);
  ...
  if (oclr.fail()){
    mowri << "Failed in " << std::setprecision(3) << elapsed_seconds << std::setprecision(6) << " [s]" << Endl;
  }
  else{
...
void Kernel::set_kernel_arg(cl_uint arg_index, size_t arg_size, const void *arg_value){
  ...
  openclutil::cl_set_kernel_arg(clkern, arg_index, arg_size, arg_value, "in set_kernel_arg of Kernel, " + hash + " index : " + std::to_string(arg_index), true);
...
void Kernel::update_times(){
  ...
  openclutil::cl_set_event_profiling_info(clevent, CL_PROFILING_COMMAND_START, sizeof(size_t), &t_start, nullptr, "in update_times", true);
  openclutil::cl_set_event_profiling_info(clevent, CL_PROFILING_COMMAND_END, sizeof(size_t), &t_end, nullptr, "in update_times", true);

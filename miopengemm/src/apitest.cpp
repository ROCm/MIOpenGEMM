                    ...
                    owrite::Writer&   mowri)
{
  ...
  setabcw::CpuMemBundle<T> cmb({gg}, toff);
  ...
  for (auto x : {Mat::E::A, Mat::E::B, Mat::E::C})
  {
    ...
    if (sizeof(T) * cmb.v_mem[x]->size() != memsize(x))
    {
      throw miog_error("Size as determined by CPU vector differs from that from memsize.");
    }

    auto mem_type_x = x == Mat::E::C ? CL_MEM_READ_WRITE : CL_MEM_READ_ONLY;

    oclutil::cl_set_buffer_from_command_queue(
      dev_mem[x], queue, mem_type_x, memsize(x), nullptr, "(runem)", true);
  ...
  for (size_t run_i = 0; run_i < n_runs; ++run_i)
  {
    ...
    if (run_i == 0 && run_accu)
    {
      ...
      // read from device
      std::vector<T> c_from_device(cmb.v_mem[Mat::E::C]->size());
      cl_event       readevent;
      ...
      // perform GEMM on CPU
      std::vector<T> c_cpu(*cmb.v_mem[Mat::E::C]);
      cpugemm::gemm<T>(
      ...
      std::memcpy(A_abs.data(), cmb.r_mem[Mat::E::A], memsize(Mat::E::A));
      for (auto& x : A_abs)
      {
        x = std::abs(x);
      }
      ...
      std::memcpy(B_abs.data(), cmb.r_mem[Mat::E::B], memsize(Mat::E::B));
      for (auto& x : B_abs)
      {
        x = std::abs(x);
      }
      ...
      std::memcpy(C_abs.data(), cmb.r_mem[Mat::E::C], memsize(Mat::E::C));
      for (auto& x : C_abs)
      {
        x = std::abs(x);
      }
      ...
      if (impl == GemmImpl::GEMM0 || impl == GemmImpl::XGEMM){
        ...
        std::cout << get_cacher().hyper_params[id].get_string() << std::endl;
      }
      
      // compare cpu and gpu results 
      accuracytests::elementwise_compare(gg,
                                         toff,
                                         cmb.r_mem[Mat::E::C],  // before,
  ...
  if (run_event_timer)
  {
    mowri << "mean time diff (event - host) " << mean_time - sum_event_times / n_to_time
              << " [ms] " << Endl;
  }

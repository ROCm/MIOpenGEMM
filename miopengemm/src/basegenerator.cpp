...
BaseGenerator::BaseGenerator(const hyperparams::HyperParams&     hp_,
  ...
  : hp(hp_), gg(gg_), dp(dp_), type(type_), kernelname("tg_" + type_)
{
}
...
void BaseGenerator::append_parameter_list_from_usage(std::stringstream& ss)
{
  ...
  if (uses_workspace == true)
  {
    ...
    // if using c, workspace is const.
    // this is a bit hacky, might have a kernel
    // which uses c and
    // modifies w too.
    if (uses_c == true)
    {
                                                 ...
                                                 bool               with_x_string)
{
  ...
  ss << "\n";
  if (withcomments)
    ss << "/* macro tiles define the pattern of C that workgroups (threads "
          "with shared local "
          "memory) process */\n";
  ss << "#define "
     << "MACRO_TILE_LENGTH" << x_string << " " << dp.at(emat_x).macro_tile_length << "\n";

  if (withcomments)
    ss << "/* number of elements in load block : MACRO_TILE_LENGTH" << x_string << " * UNROLL */\n";
  ss << "#define "
     << "N_ELEMENTS_IN" << x_string << "_UNROLL " << dp.at(emat_x).n_elements_in_unroll << "\n";
  ...
  ss << "#define "
     << "N_GROUPS" << x_string << " " << dp.at(emat_x).n_groups << "\n";
  ...
  if (dp.main_use_edge_trick != 0)
  {
    ...
    ss << "#define "
       << "PRESHIFT_FINAL_TILE" << x_string << " " << dp.at(emat_x).preshift_final_tile << "\n";
  }
}
...
std::string BaseGenerator::get_time_string()
{
  ...
  ss <<
    R"(/* ****************************************************************************
* This )"
     << type << " kernel string was generated on " << std::ctime(&generation_time) <<
    R"(**************************************************************************** */)";
  return ss.str();
}

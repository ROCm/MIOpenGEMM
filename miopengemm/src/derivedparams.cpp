...
void DerivedParams::reset_cw_params(nsHP::eMat emat_x){
  if (emat_x == nsHP::matB && ptr_hp->at(nsHP::matA).vs[nsHP::WOS] != 0 && adps.cw_n_elements == uninitialised_unsigned){
  ...
  if (ptr_hp->at( emat_x ).vs[nsHP::WOS] == 1){
    at(emat_x).cw1_smallest_possible_ldx = ptr_gg->coal_is_pll_k(emat_x) ? ptr_gg->k : ptr_gg->get_non_k_dim(emat_x);
  ...
  else if (ptr_hp->at( emat_x ).vs[nsHP::WOS] == 2){
    ...
    cw2_n_macro_tiles_pll_unroll = ptr_gg->k / ptr_hp->at(nsHP::matC).vs[nsHP::UNR] + ((ptr_gg->k % ptr_hp->at(nsHP::matC).vs[nsHP::UNR]) != 0);
  ...
  at(emat_x).cw_global_offset = (emat_x == nsHP::matB && ptr_hp->at(nsHP::matA).vs[nsHP::WOS] != 0) ? at(nsHP::matA).cw_n_elements : 0;
...
void DerivedParams::reset_ga3_params(){
  ...
  if (main_split_on_k == 1){
    ...
    static_cast<unsigned>(std::floor(std::sqrt(static_cast<double>(ptr_hp->at(nsHP::matC).vs[nsHP::NAW]) / static_cast<double>(ptr_hp->at(nsHP::matC).vs[nsHP::ICE]))));
  }
  else if (main_split_on_k == 0){
    ga3_super_column_width = 
    static_cast<unsigned>(std::floor(std::sqrt(static_cast<double>(ptr_hp->at(nsHP::matC).vs[nsHP::NAW]))));
  }
  else{
...
DerivedParams::DerivedParams(const hyperparams::HyperParams & hp_, const Geometry & gg_, std::string s):ptr_hp(&hp_), ptr_gg(&gg_){
...
DerivedParams::set_fragile(){
  ...
  auto grid_size_tuple = nsMAC::get_mac_grid(ptr_hp->at(nsHP::matC).vs[nsHP::MAC], ptr_hp->at(nsHP::matC).vs[nsHP::SKW]);
  ...
  at(nsHP::matA).macro_tile_length = grid_size[nsHP::matA] * ptr_hp->at(nsHP::matA).vs[nsHP::MIC];
  at(nsHP::matB).macro_tile_length = grid_size[nsHP::matB] * ptr_hp->at(nsHP::matB).vs[nsHP::MIC];
  
      
      
  for (auto emat_x : {nsHP::matA, nsHP::matB}){
    at(emat_x).preshift_final_tile = 1 + (ptr_gg->get_non_k_dim(emat_x) - 1) % at(emat_x).macro_tile_length;
    at(emat_x).n_groups = ptr_gg->get_non_k_dim(emat_x) / at(emat_x).macro_tile_length + (at(emat_x).preshift_final_tile != at(emat_x).macro_tile_length);
    at(emat_x).main_macro_tile_length_and_pad = at(emat_x).macro_tile_length + ptr_hp->at( emat_x ).vs[nsHP::PAD];
    
    
    at(emat_x).main_n_elements_in_padded_unroll = at(emat_x).main_macro_tile_length_and_pad * ptr_hp->at(nsHP::matC).vs[nsHP::UNR];
  ...
  main_micro_tile_area = ptr_hp->at(nsHP::matB).vs[nsHP::MIC] * ptr_hp->at(nsHP::matA).vs[nsHP::MIC];
  ...
  for (auto emat_x : {nsHP::matA, nsHP::matB}){
    ...
    if (ptr_gg->m < at(nsHP::matA).macro_tile_length){
      set_status_ss  << "m < aps.macro_tile_length, not considering this kernel. ";
    }
    
    /* check - 4 : the macro tile is too wide */
    else if (ptr_gg->n < at(nsHP::matB).macro_tile_length){
      set_status_ss << "m < bps.macro_tile_length, not considering this kernel. ";
    }
    
    at(emat_x).n_elements_in_unroll = at(emat_x).macro_tile_length * ptr_hp->at(nsHP::matC).vs[nsHP::UNR];
    at(emat_x).main_n_elements_to_load_per_workitem = at(emat_x).n_elements_in_unroll / main_n_work_items_per_workgroup;  
    
    if (ptr_hp->at( emat_x ).vs[nsHP::WOS] == 2){
      at(emat_x).cw2_n_elements_to_load_per_workitem = at(emat_x).n_elements_in_unroll / at(emat_x).cw2_local_work_size;  
    }
    
    if (ptr_hp->at( emat_x ).vs[nsHP::WOS] != 0){
      reset_cw_params(emat_x);
      required_workspace += at(emat_x).cw_n_elements; //cw1_target_ldx*ptr_gg->get_uncoal(x);
    }
  
    /* check 0 : macro tile not too large */  
    if (ptr_gg->get_non_k_dim(emat_x) < at(emat_x).macro_tile_length){
      set_status_ss << "ptr_gg->get_non_k_dim( " << matChars[emat_x] << " )  < at ( " << matChars[emat_x] << " ).macro_tile_length, this means the tile is too big to work with  " << matChars[emat_x] << " . not considering this kernel. ";
    }
  ...
  if (ptr_gg->workspace_size < required_workspace){
    set_status_ss << "ptr_gg->workspace_size ( " << ptr_gg->workspace_size << " ) is less then the required workspace ( " << required_workspace << " ). "; 
  }
  ...
  for (auto emat_x : {nsHP::matA, nsHP::matB}){
    ...
    if (ptr_hp->at( emat_x ).vs[nsHP::WOS] == 2){// && at(emat_x).n_elements_in_unroll % at(emat_x).cw2_local_work_size != 0){
  ...
  for (auto emat_x : {nsHP::matA, nsHP::matB}){
    ...
    auto tup = tiling::get_tileability(at(emat_x).macro_tile_length, ptr_hp->at(nsHP::matC).vs[nsHP::UNR], at(emat_x).main_n_elements_to_load_per_workitem);
    if (std::get<0>(tup) == false){
      return tup;
    }
    
    if (ptr_hp->at( emat_x ).vs[nsHP::WOS] == 2){
      tup = tiling::get_tileability(at(emat_x).macro_tile_length, ptr_hp->at(nsHP::matC).vs[nsHP::UNR], at(emat_x).cw2_n_elements_to_load_per_workitem);
      if (std::get<0>(tup) == false){
        return tup;
      }
    }
  ...
  if (ptr_hp->at(nsHP::matC).vs[nsHP::UFO] == nsHP::yes){
    if (ptr_gg->k <= ptr_hp->at(nsHP::matC).vs[nsHP::UNR]){
...
DerivedParams::DerivedParams(const hyperparams::HyperParams & hp_, const Geometry & gg_): ptr_hp(&hp_), ptr_gg(&gg_) {
  ...
  for (auto emat_x : {nsHP::matA, nsHP::matB}){
    ...
    tiling::set_tile_dimensions(at(emat_x).main_micro_tile_perp_unroll, at(emat_x).main_micro_tile_pll_unroll, at(emat_x).macro_tile_length, ptr_hp->at(nsHP::matC).vs[nsHP::UNR], at(emat_x).main_n_elements_to_load_per_workitem, ptr_hp->at( emat_x ).vs[nsHP::PLU] == 0);
    
    if (ptr_hp->at( emat_x ).vs[nsHP::WOS] == 2){
      tiling::set_tile_dimensions(at(emat_x).cw2_micro_tile_perp_unroll, at(emat_x).cw2_micro_tile_pll_unroll, at(emat_x).macro_tile_length, ptr_hp->at(nsHP::matC).vs[nsHP::UNR], at(emat_x).cw2_n_elements_to_load_per_workitem, at(emat_x).cw2_load_pll_to_unroll == 0);
    }
  } 
  
  
  main_split_on_k = ptr_hp->at(nsHP::matC).vs[nsHP::ICE] == 1 ? 0 : 1;
  main_does_beta_c_inc = main_split_on_k == 1 ? 0 : 1; 
  
  if (ptr_hp->at(nsHP::matC).vs[nsHP::ICE] == 1){
  ...
  pragma_unroll_string = ptr_hp->at(nsHP::matC).vs[nsHP::PUN] == 1 ?  "#pragma unroll\n" : "" ;
  
  effective_k_varies_string = ptr_hp->at(nsHP::matC).vs[nsHP::UFO] == 0 ? "__K__" : "k_plus_offset";
  t_float = ptr_gg->derived.float_size_bits == 32 ? "float" : "double";
  
  main_n_work_groups = ptr_hp->at(nsHP::matC).vs[nsHP::ICE] * 
  ((ptr_gg->m/at(nsHP::matA).macro_tile_length) + (ptr_gg->m%at(nsHP::matA).macro_tile_length != 0)) * 
  ((ptr_gg->n/at(nsHP::matB).macro_tile_length) + (ptr_gg->n%at(nsHP::matB).macro_tile_length != 0));
  
  main_global_work_size = main_n_work_groups * main_n_work_items_per_workgroup;
  
  for (auto emat_x : {nsHP::matA, nsHP::matB}){

    at(emat_x).main_n_micro_in_macro = at(emat_x).macro_tile_length / ptr_hp->at( emat_x ).vs[nsHP::MIC];
    at(emat_x).main_n_micro_tiles_pll_unroll = ptr_hp->at(nsHP::matC).vs[nsHP::UNR] / at(emat_x).main_micro_tile_pll_unroll;
    at(emat_x).main_c_interweave_stride = ptr_hp->at( emat_x ).vs[nsHP::MIW] == 0 ? 1 : at(emat_x).main_n_micro_in_macro;  
    
    if (ptr_hp->at( emat_x ).vs[nsHP::WOS] == 2){
      at(emat_x).cw2_n_micro_tiles_pll_unroll = ptr_hp->at(nsHP::matC).vs[nsHP::UNR] / at(emat_x).cw2_micro_tile_pll_unroll;
      at(emat_x).cw2_n_micro_tiles_perp_unroll = at(emat_x).macro_tile_length / at(emat_x).cw2_micro_tile_perp_unroll;
    }
  }

  if (ptr_hp->at(nsHP::matC).vs[nsHP::GAL] == 3){
    reset_ga3_params();
  }

  /* these guys are hyper params, with a check if not optional ? */
  main_use_edge_trick = (ptr_gg->m%at(nsHP::matA).macro_tile_length == 0 && ptr_gg->n%at(nsHP::matB).macro_tile_length == 0) ? 0 : 1;
  main_final_fractional_unroll = (ptr_hp->at(nsHP::matC).vs[nsHP::UFO] == 1 || ptr_gg->k%ptr_hp->at(nsHP::matC).vs[nsHP::UNR] != 0) ? 1 : 0;

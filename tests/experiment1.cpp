...
int main(){
  ...
  auto geometries = MOOMOOMOOGEMM::get_small_growing_geometries();
  ...
  if (n_iterations_smallgrowing > 0){
    MOOMOOMOOGEMM::FindParams find_params(allotted_time, n_iterations_smallgrowing, n_runs_per_kernel, sumstat);
    MOOMOOMOOGEMM::run_find_experiments(geometries, v_constraints, find_params, verbose, basedir, verbose_outer, fn_outer);
  }
  ...
  geometries = MOOMOOMOOGEMM::get_small_deepbench_geometries(small_threshold);
  v_constraints = {"", "C_ICE1"};
  if (basedir != "")
    subdir = basedir + "smalldeep/";
  
  if (n_iterations_smalldeep > 0){
    MOOMOOMOOGEMM::FindParams find_params(allotted_time, n_iterations_smalldeep, n_runs_per_kernel, sumstat);
    MOOMOOMOOGEMM::run_find_experiments(geometries, v_constraints, find_params, verbose, basedir, verbose_outer, fn_outer);
  }


  std::cout << "\nLARGEDEEP EXPERIMENTS : HOW DO WE DO ON THE LARGE DEEPBENCH PROBLEMS (WITHOUT ICE ALLOWED) ?" << std::endl;
  geometries = MOOMOOMOOGEMM::get_large_deepbench_geometries(small_threshold);
  v_constraints = {""};
  if (basedir != "")
    subdir = basedir + "largedeep/";

  if (n_iterations_largedeep > 0){
    MOOMOOMOOGEMM::FindParams find_params(allotted_time, n_iterations_largedeep, n_runs_per_kernel, sumstat);
    MOOMOOMOOGEMM::run_find_experiments(geometries, v_constraints, find_params, verbose, basedir, verbose_outer, fn_outer);
  }
  ...
  geometries = MOOMOOMOOGEMM::get_square_geometries();
  v_constraints = {""};
  if (basedir != "")
    subdir = basedir + "square/";
  if (n_iterations_square > 0){
    MOOMOOMOOGEMM::FindParams find_params(allotted_time, n_iterations_square, n_runs_per_kernel, sumstat);
    MOOMOOMOOGEMM::run_find_experiments(geometries, v_constraints, find_params, verbose, basedir, verbose_outer, fn_outer);
  }


  std::cout << "\nPROBLEM EXPERIMENTS : FOR GEOMETRIES WHICH ARE GIVING US A HARD TIME :-{|>" << std::endl;
  geometries = MOOMOOMOOGEMM::get_problem_geometries();
  v_constraints = {""};
  if (basedir != "")
    subdir = basedir + "problemgeoms/";
  if (n_iterations_problem_geometries > 0){
    MOOMOOMOOGEMM::FindParams find_params(allotted_time, n_iterations_problem_geometries, n_runs_per_kernel, sumstat);
    MOOMOOMOOGEMM::run_find_experiments(geometries, v_constraints, find_params, verbose, basedir, verbose_outer, fn_outer);
  }

...
void set_usage(){
  ...
  uses_a = (hp.at(matA).vs[WOS] == 0) ? true : false;   //this worked. propogate.
  uses_b = (hp.at(matB).vs[WOS] == 0) ? true : false;
  uses_c = true;
  uses_workspace = (hp.at(matA).vs[WOS] + hp.at(matB).vs[WOS]) == 0  ? false : true;
  uses_alpha  = true;
...
group_id_a = (group_id_xy  - (N_GROUPS_B - LAST_SUPER_COLUMN_WIDTH)*N_GROUPS_A) / LAST_SUPER_COLUMN_WIDTH;
    ...
    err_ss << "Invalid group_allocation parameter : " << hp.at(matC).vs[GAL] << ". It should be one of 1/2/3.";
...
void append_load_for_perp(char X, std::stringstream & ss){
  std::string bound_string = hp.at(X).vs[LIW] == 0 ? std::string("MICRO_") + X + "_TILE_PERP_UNROLL" : std::string("MACRO_TILE_LENGTH_") + X;
  std::string increment_string = hp.at(X).vs[LIW] == 0 ? "++mu_perp_i" : std::string("mu_perp_i += MACRO_TILE_LENGTH_") + X + "/MICRO_" + X + "_TILE_PERP_UNROLL";
  append_loop_var_bound_incr(ss, "mu_perp_i", bound_string, increment_string);
}

void append_load_for_pll(char X, std::stringstream & ss){
  std::string bound_string = hp.at(X).vs[LIW] == 0 ? std::string("MICRO_") + X + "_TILE_PLL_UNROLL" : "UNROLL";
  std::string increment_string = hp.at(X).vs[LIW] == 0 ? "++mu_pll_i" : std::string("mu_pll_i += UNROLL/MICRO_") + X + "_TILE_PLL_UNROLL";
...
void append_for_loops_for_c_write_open(std::stringstream & ss){
  ...
  hp.at('A').vs[MIW] == 0 ? "MICRO_TILE_LENGTH_A" : "MACRO_TILE_LENGTH_A", 
  hp.at('A').vs[MIW] == 0 ? "++row" : "row += N_MICRO_IN_MACRO_A");
  ss << " {\n";
  
  ss  << dp.pragma_unroll_string;
  append_loop_var_bound_incr(ss, "col",
  hp.at('B').vs[MIW] == 0 ? "MICRO_TILE_LENGTH_B" : "MACRO_TILE_LENGTH_B", 
  hp.at('B').vs[MIW] == 0 ? "++col" : "col += N_MICRO_IN_MACRO_B");
  ss << " {\n";
}
...
std::string get_c_work_item_next(char X){
  return (hp.at(X).vs[MIW] != 0) ? "1" : (std::string("MICRO_TILE_LENGTH_") + X);
}
...
void append_group_allocation_defn_string(std::stringstream & ss){
  ss << "#define GROUP_ALLOCATION " << hp.at(matC).vs[GAL];
  if (hp.at(matC).vs[GAL] == 3){
    ...
    ss << "#define N_TARGET_ACTIVE_WORKGROUPS " << hp.at(matC).vs[NAW] << "\n";
  }
}
...
#define G_UNROLL )" << hp.at(matC).vs[ICE]*hp.at(matC).vs[UNR] << " // N_WORK_ITEMS_PER_C_ELM*UNROLL";
  }
}
...
void append_id_string_sym(std::stringstream & ss, char x){
  ...
  if (hp.at(x).vs[WOS] == 1 || hp.at(x).vs[WOS] == 2){
  ...
  if (dp.main_use_edge_trick != 0 && hp.at(x).vs[WOS] != 2 ) {
...
void add_predefine_chiral(char x, std::stringstream & ss){
  ...
  ss << "#define LOAD_TO_LDS_INTERWOVEN_" << x << " " << hp.at(x).vs[LIW] << "\n";
  ...
  ss << "#define C_MICRO_TILES_INTERWOVEN_" << x << " " << hp.at(x).vs[MIW] << "\n";
...
/* TODO : rerereconsider assignment of workitems to load and math regions, there may be some sweet overlap spot where values automatically in registers for math (?) */
  ...
  ss << "#define UNROLL_FOR_OFFSET " << hp.at(matC).vs[UFO] << "\n";
  ...
  ss << "#define N_WORK_ITEMS_PER_C_ELM " << hp.at(matC).vs[ICE] << "\n";
 ...
 * 3 : by rows within super-column  */
  ...
  ss << "#define PRAGMA_UNROLL_FORLOOPS " << hp.at(matC).vs[PUN] << "\n";

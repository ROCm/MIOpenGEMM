...
public:
  ...
  Gemini(gemmgeometry::Geometry gg, const TFloat * a, const TFloat * b, TFloat * c, TFloat alpha, TFloat beta, std::string outputfilename):gg(gg), a(a), b(b), c(c), alpha(alpha), beta(beta), outputfilename(outputfilename), mowri(true, outputfilename.compare("") != 0, outputfilename)  {
